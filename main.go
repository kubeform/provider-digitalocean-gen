...
func main() {
	apisPath := flag.String("apis-path", "<empty>", "path to generate the apis. Pass empty string to use default path <$GOPATH/kubeform.dev/provider-digitalocean-api")
	controllerPath := flag.String("controller-path", "<empty>", "path to generate the controller. Pass empty string to use default path <$GOPATH/kubeform.dev/provider-digitalocean-controller")
	flag.Parse()
